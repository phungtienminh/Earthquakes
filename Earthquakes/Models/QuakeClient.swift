...
actor QuakeClient {
    private var quakeCache: NSCache<NSString, CacheEntryObject> = NSCache()
    ...
    func quakeLocation(from url: URL) async throws -> QuakeLocation {
        ...
        let task = Task<QuakeLocation, Error> {
            let data = try await self.downloader.httpData(from: url)
            let quakeLocation = try decoder.decode(QuakeLocation.self, from: data)
